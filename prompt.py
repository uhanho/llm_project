...
load_dotenv()
def prompt_create(city, date, date2, number, prompt):
    database = OracleVS.Database()
    ...
    template = """Create an hourly travel plan based on the following information and prompt.
        ...
        Prompt: {prompt}
        Recommend the places based on these: {context}
    ...
    while(True):
        chain = {"context": retriever, "prompt": RunnablePassthrough(), "city": city, "date": date, "date2": date2, "number": number} | prompt_template | llm | StrOutputParser() | llm | StrOutputParser()
...
prompt_create("도쿄", "2024-07-06", "2024-07-09", 1, "여유로운 여행을 원해")